 ...
 *
 * Czech stemmer-removes case endings form nouns and adjetives, possesive adj. endings from names and takes care of palatalisation
 */
public class CzechStemmer {
	...
	private static void removeInformal(StringBuffer buffer) {
		int len = buffer.length();
		
		if (len > 2 && buffer.substring(len - 2, len).equals("ej")){
			buffer.replace(len - 2, len, "ý");
		}
	}

	private static void palatalise(StringBuffer buffer) {
		int len = buffer.length();
	...
	private static void removePossessives(StringBuffer buffer) {
		int len = buffer.length();
	...
	private static void removeCase(StringBuffer buffer) {
		int len = buffer.length();
		// 
		if ((len > 7)
